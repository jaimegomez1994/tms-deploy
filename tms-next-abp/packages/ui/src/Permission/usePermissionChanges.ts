...
import { PermissionGrantInfoDto } from '@tms_next_abp/proxy';
import { Permissions } from '../utils';

export type UsePermissionsChangesProps = {
    permissions: PermissionGrantInfoDto[];
    type: 'identity' | 'tenant' | 'feature' | 'setting' | 'administracion';
};
...
const helper = (
    ...
    const parent = data.find(
        (f) =>
            !f.parentName &&
            (f.name === permission || f.name === 'Operacion.Documentadores')
    );
    ...
    const onCurrentPermissionChanges = useCallback(
        (idx: number) => {
            ...
            if (type === 'identity') {
                ...
                helper(data, selectedData, Permissions.ADMIN, setData);
                helper(data, selectedData, Permissions.OPER, setData);
            } else {
                throw new Error(
